...
Каталог рецептов доступен авторизованным пользователям с подтверждённым email — в навигации
ему отвечает пункт меню **«Рецепты»**.

* **Список.** Общий каталог всех рецептов с постраничной навигацией (по 12 на странице);
  карточка открывает отдельный рецепт.
* **Просмотр.** Любой рецепт можно открыть целиком — состав, шаги приготовления, время и число
  порций — независимо от того, кто его добавил.
* **Создание.** Новый рецепт добавляется через форму с динамическими списками шагов и ингредиентов.
* **Редактирование и удаление.** Доступны только автору рецепта: чужие рецепты видны для просмотра,
  но кнопки правки и удаления показываются лишь владельцу.
