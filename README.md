...
Административная панель доступна по адресу **`/admin`** пользователям с правами администратора
(и подтверждённым email). Обычным пользователям и гостям вход в панель закрыт.

* **Рецепты.** Полный доступ ко всем рецептам всех авторов: список, создание, редактирование
  (включая шаги и ингредиенты) и удаление.
* **Пользователи.** Просмотр списка и карточек пользователей (без управления учётными записями).

Права администратора выдаются существующему пользователю командой:

```bash
./vendor/bin/sail artisan app:grant-admin user@example.com
```
